...
1. **Read the playbook.** Open `docs/agents/orchestration.md` for the roster
   (9 lenses, routing globs), loop shape, verdict schema, PR-body record, and
...
3. **Run the loop (manual path):** dispatch the Worker with an explicit spec +
   acceptance criteria → route the panel (4 always-on lenses: correctness,
   security, conventions, architecture; add design/accessibility/copy,
   performance, and data when the changed files match the roster's globs) →
   spawn a **fresh** specialist per lens, in parallel → on any
   `CHANGES_REQUESTED`, send the **original** Worker back to fix or rebut every
   finding across the panel → re-check with a fresh instance of **only each
   objecting lens**, given the prior findings and the worker's responses.
...
into the PR's `## Adversarial review` section.

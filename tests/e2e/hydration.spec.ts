...
/**
 * Regression guard for app hydration (the "no-JS site" failure mode).
 *
 ...
 * client bundle, which runs app/client.tsx's hydrateRoot (which in turn
 * imports `getRouter` from app/router.tsx). The TanStack Start Vite plugin
 * wires the entry automatically; if that wiring breaks (e.g. app/router.tsx
 * stops exporting `getRouter`, or the client entry is dropped), no client
 * script is injected and nothing hydrates: voting, the add-listing form, the
 * taxonomy filter, sort, and SPA <Link> navigation are all inert (a no-JS
 * site).
 *
 ...
 * build. The client-entry `src` differs by mode: dev injects the virtual
 * module `/@id/virtual:tanstack-start-dev-client-entry`, prod injects the
 * hashed `/assets/index-*.js` bundle — the assertion accepts both and then
 * fetches the referenced src to prove the entry asset actually resolves.
 */
...
test("the app hydrates: a <Link> does client-side navigation, not a full reload", async ({
  ...
  // effect (see helpers.ts for why a `__TSR_ROUTER__` signal would be too
  ...
  // `/` is the directory; its floating "Add listing" FAB links to the
  // add-listing route via TanStack Router's <Link>. Client-side navigation

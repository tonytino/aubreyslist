...
/**
 * Post-creation link editing on the listing detail page.
 *
...
test.describe("edit listing links (wiki-style, signed-in)", () => {
  ...
  test("signed-in user adds a website link from the detail page", async ({
    ...
    // can otherwise be lost to a mid-save document reload. Deliberately no
    // `page.reload()` here — issuing our own navigation can collide with that
    // in-flight framework reload and destabilise the interactions below; the
    // presence assertions retry until whichever refresh wins renders the
    // committed row.
  ...
  test("removing a legacy menu link sticks (no fallback resurrection)", async ({
    ...
    // A legacy row: menu link only in the legacy menu_url column. Clearing
    ...
    // Two barriers — both are needed:
    //
    // 1. Dialog-close: the dialog closes only in the mutation's onSuccess,
    //    i.e. after the server deleted the typed row and cleared the legacy
    //    menu_url column. While it is open the modal marks the background
    //    aria-hidden, so the Links assertion below would pass vacuously. This
    //    gates on client-observable success.
    //
    // 2. DB-side barrier: a framework-initiated mid-save document reload can
    //    both discard the client-side refetch and server-render the page from
    //    a read that raced the still-committing mutation — so even after the
    //    dialog closes, the following `page.reload` can SSR a stale row and
    //    resurrect the legacy Menu link (red at CI's Neon latency). Poll the
    //    column this spec guards — the write contract — until it is actually
    //    null before reloading, closing the race window.
    await expect(page.getByRole("dialog")).toHaveCount(0);
    ...
    // that cannot race the mutation with the barriers in front.

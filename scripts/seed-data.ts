...
/**
 * Curated Denver-metro gluten-free / celiac seed data (AUB-31).
 *
 * WHAT THIS IS: a starter set of real, currently-operating Denver-proper (and
 * close-suburb) spots with a public gluten-free reputation, so the directory has
 * density before real users arrive. Each entry is resolved to a REAL Google Place
 * ID + coordinates at seed time (see `scripts/seed.ts`), and each gets one or more
 * GF-attribute "labels" SUGGESTED by the curator bot ("Aubrey's Bot") — surfaced
 * as a "Suggested by Aubrey's Bot" badge, kept OUT of the community confirm/dispute
 * counts, and cleared the instant a real user attests (ADR-007 honesty).
 *
 ...
 * skipped and logged rather than guessed. Add/curate freely and re-run — the seed
 * is idempotent (dedup on Place ID; a claim a real user has engaged with is never
 * re-suggested).
 */
...
/** One curated seed entry: a Places query + the labels the bot suggests for it. */

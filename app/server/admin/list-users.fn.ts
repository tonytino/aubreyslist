...
/**
 * Client-callable admin user-directory server function.
 *
 * Entry point the role-management section calls to list accounts. The
 * db-touching logic lives in the server-only `./list-users` module, referenced
 ...
 *
 * Auth is enforced server-side inside `listUsers`
 * (`requireCurrentRole("admin")`) — anonymous → 401, any non-admin (including
 * a moderator) → 403. UI gating is never the control.
 */

...
// Security response headers on every /api response. Mounted first so it
// wraps all downstream handlers and stamps headers even on error/404
// responses. The same header set is applied to SSR/document responses in
// `app/start.ts` — see `app/server/security/headers.ts`.
app.use("*", honoSecurityHeaders());

// CSRF defense-in-depth: reject state-changing cross-origin requests with
// 403 before any route/DB work. Central here for /api mutations; the
...
// Google Place photo media proxy (ADR-014) — resolves a transient photo
// token to Google's short-lived media URL server-side and 302s to it.

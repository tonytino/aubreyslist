/**
 * Pure date-formatting helpers for incident reports. Kept out of the React
 * components so they are unit-testable and reusable by every incident surface.
 *
 * Incident dates are stored as calendar dates (`YYYY-MM-DD`, the `date` column).
 * Parse them at UTC midnight so a date never drifts by a day across timezones —
 * a "got glutened on the 1st" must read as the 1st everywhere.
 */
 ...
 *
 * Recency is UTC-calendar-based — both the incident date and `now` are floored to
 * UTC midnight before measuring the gap, matching `isRecentIncident`, so SSR and
 * client render the same phrase and a UTC-7 report near midnight doesn't read
 * off-by-one. Pass `now` once (resolved server-side and threaded down) rather than
 * relying on the default, which would differ between server and browser.
 */

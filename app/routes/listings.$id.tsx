...
import { useListingPreviewSrc } from "~/listings/photo-preview-state";
      ...
      <header className="relative overflow-hidden rounded-card border border-border bg-surface shadow-sm">
        ...
        <div className="relative aspect-[16/9] bg-gradient-to-br from-brand to-brand-strong sm:aspect-[21/9]">
          ...
          {/* Render-time place photo + its attribution line (client-side query,
              ...
              listing B's photo. `previewSrc` (router state from the browse
              card, absent on a direct visit/refresh) lets the hero blur-up
              from the card's already-cached photo instead of starting blank. */}
          <HeroPhoto key={listing.id} listingId={listing.id} previewSrc={previewSrc} />

...
| `VITE_GOOGLE_MAPS_BROWSER_KEY` | Optional | human (AUB-217) | **Public, client-side** (see below) Google Maps browser key for the directory map (AUB-111). Referrer-restricted in the Google console; NOT in `app/env.ts` — read via `app/lib/public-env.ts`. Absent → the map view renders its CSS-placeholder fallback. |

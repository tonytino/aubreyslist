...
| `VITE_GOOGLE_MAPS_BROWSER_KEY` | Optional | human (AUB-217) | **Public** browser key for Maps JavaScript API + Maps Embed API (ADR-014). Deliberately client-exposed — its security model is HTTP-referrer restriction + API restriction to those two Maps APIs only; it must never be able to call Places. Distinct from `GOOGLE_PLACES_API_KEY` (server-only); never cross-use the two. Optional by design: map surfaces degrade gracefully to the CSS placeholder / Google Maps deep-link when absent. **Not yet in `app/env.ts` / `.env.example`** — wired when the first map surface (AUB-111/AUB-216) lands; provisioned via AUB-217. |
